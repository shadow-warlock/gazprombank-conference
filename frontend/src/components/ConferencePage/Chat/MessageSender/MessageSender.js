...
class MessageSender extends Component {
    ...
    send() {
        if (this.state.message !== '') {
            ...
            if (this.props.reply) data.replyTo = this.props.reply.id;
            axios
                .post(API.CHAT_MESSAGE(this.props.chatId), data, AXIOS_CONFIG)
                .then(() => {
    ...
    getReplyElement() {
        if (this.props.reply) {
            let text =
                this.props.intl.formatMessage({ id: 'reply_to' }) +
                this.props.reply.user.name +
                ' ' +
                this.props.reply.user.surname;
            return (
                <>
                    {text}{' '}
                    <FontAwesomeIcon
            ...
            return <FormattedMessage id={'your_question'} />;
        }
    }
