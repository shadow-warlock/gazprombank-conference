...
class Message extends Component {
    ...
    like() {
        axios
            .post(API.MESSAGE_LIKE(this.props.message.id), {}, AXIOS_CONFIG)
            .catch((e) => {
                console.error(e);
            });
    }
    ...
    unlike() {
        axios
            .delete(API.MESSAGE_LIKE(this.props.message.id), AXIOS_CONFIG)
            .catch((e) => {
                console.error(e);
            });
    }
    ...
    getReplyToElement() {
        ...
        if (replyTo) {
            return (
                <span className={'reply_to'} onClick={this.scrollToMessage}>
                    {replyTo.user.name + ', '}
                </span>
            );
        } else {
    ...
    removeMessage() {
        if (
            window.confirm(
                this.props.intl.formatMessage({ id: 'delete_confirm' })
            )
        ) {
            axios
                .delete(API.MESSAGE(this.props.message.id), AXIOS_CONFIG)
    ...
    render() {
        ...
        return (
            <div
                ...
                id={'msg_' + this.props.message.id}
            >
                <div className={'message'}>
                    <div>
                        <p className={'uppercase bold color_red'}>
                            {message.user.name} {message.user.surname}
                        </p>
                        <p className={'text'}>
                            {this.getReplyToElement()} {message.text}
                        </p>
                    </div>
                    <div>
