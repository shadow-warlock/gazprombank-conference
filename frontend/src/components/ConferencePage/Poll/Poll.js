...
export default class Poll extends Component {
    ...
    send(question) {
        if (this.state.answer !== '') {
            axios
                .post(
                    API.QUESTION_ANSWER(question.id),
        ...
        if (question.variants) {
            ...
            return (
                <>
                    ...
                    />
                    <Button onClick={() => this.send(question)}>
                        Ответить
                    </Button>
                </>
            );
    ...
    getPoll() {
        ...
        if (question === null) {
            return (
                <p className={'uppercase font_size_big bold'}>
                    Спасибо за ответ!
                </p>
            );
        } else {
            return (
                <>
                    <p className={'font_size_big uppercase bold'}>
                        {question.question}
                    </p>
    ...
    findActualQuestion() {
        for (let key in this.props.poll.questions) {
            ...
            let isMyAnswer = question.answers.find(
                (answer) => answer.user.id === this.props.user.id
            );
            if (!isMyAnswer) {
                return question;
            }
            ...
            <div className={'poll color_white'}>
                <p className={'bold font_size_very_big uppercase'}>
                    ...
                    {this.state.time !== null && (
                        <span className={'font_size_default'}>
                            Закончится через {this.state.time}...
                        </span>
                    )}
                </p>
                {this.getPoll()}
